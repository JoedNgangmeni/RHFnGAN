...
def sortAggData(entry_path): 
    ...
    :doc-author: Trelent
    """
    print(f"Sorting data from {entry_path} ...")    
    data = pd.read_csv(entry_path, sep='\t', header=0)
...
def avgAggData(data: pd.DataFrame):
    print(f"Averaging data...") 

    newDF = pd.DataFrame(columns=data.columns)
    for num_trees in data['numTrees'].unique():  
        for tree_depth in data['treeDepth'].unique():
            # print(f'{num_trees}\t{tree_depth}')
            subset = data.loc[(data['numTrees'] == num_trees) & (data['treeDepth'] == tree_depth)]
            if not subset.empty:
                group_mean = subset.mean()
                group_mean_df = group_mean.to_frame().T

                newDF = pd.concat([newDF, group_mean_df], ignore_index=True)
